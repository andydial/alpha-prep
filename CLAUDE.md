...
✅ **Wrong answer key served with no correct option — FIXED (24 August 2026).** Aarav was shown a pool-and-path question keyed to `A) 87 m²` whose own explanation worked out 66 m² — and 66 was not among the four options. Two independent holes:
1. **Only `working` was validated against the key, never `explanation`** — and the explanation is where the model does its careful second pass. The old check also merely asked "does the key's number appear in the text?", which the sentence *"the answer is A) 87 m² is wrong"* satisfies. `findQuestionDefect` in `answerCheck.ts` is now the single gate for both generators: format, option count, key-in-options, non-empty working, answer-follows-working, and — new — `contradictsDeclaredAnswer` (does any *declared* conclusion disagree with the key?) and `hasSelfCorrection` (did the model visibly change its mind?). Each retry is told what was wrong via `DEFECT_FEEDBACK` instead of repeating the identical request.
2. **`evaluateAnswer` failed silently.** One attempt, 700 max_tokens against measured 430–600 output, and on any failure `localFallback()` returned the *unverified* key with empty feedback and no signal. It now retries, uses 1600 tokens, treats `stop_reason: 'max_tokens'` as a failure, and sets `markerUnavailable` so the UI can say the answer was not checked. `ExplanationPanel` warns on that and on `optionsFaulty` (verified answer not among the options).

Regression tests: `src/lib/questionIntegrity.test.ts` (validators, built from the real artifact) and `src/lib/generateQuestionGate.test.ts` (gate wired into `generateQuestion`, marker retry — API stubbed, deterministic).
...
✅ **Clock pauses when the student is waiting on us — DONE (August 2026).** The countdown used to run through question generation and through the second-opinion marking of a wrong answer, both of which are Anthropic round trips: on a 60-second-per-question pace that was quietly spending whole questions' worth of the test on spinners. `PausableClock` in `src/lib/sessionTimer.ts` owns the arithmetic (shift the deadline by the length of the pause; a paused clock never expires), `useCountdown` exposes `paused` / `setPaused`, and `Study.tsx` drives it from one derived boolean — `loading || evaluating || onBreak` — so overlapping callers cannot double-count. The clock deliberately keeps running through the block-1→block-2 transition and while the explanation is on screen.

✅ **Student can pause the test — DONE (August 2026).** A Pause button on the study and Written Expression screens opens `PauseOverlay`, which stops the clock and covers the page opaquely — a translucent scrim would turn "pause for dinner" into unlimited thinking time. Resume returns to the same question with the same time remaining and nothing typed lost, because the overlay sits over the page rather than replacing it. Study also registers a `beforeunload` guard: a reload would lose the in-memory question and blueprint, so an accidental one now costs a confirmation.
